import type { Reactified, ReactifyOptions } from '../reactify/reactify';
import type { Fn } from '../utils/types';
import { reactify } from '../reactify/reactify';

export type ReactifyNested<T, Keys extends keyof T = keyof T, S extends boolean = true> = {
  [K in Keys]: T[K] extends Fn ? Reactified<T[K], S> : T[K];
};
...
export function reactifyObject<T extends object, S extends boolean = true>(
  obj: T,
...
export function reactifyObject<T extends object, S extends boolean = true>(
  obj: T,
  optionsOrKeys: ReactifyObjectOptions<S> | (keyof T)[] = {},
): ReactifyObjectReturn<T, keyof T, S> {
  let keys: string[] = [];
  ...
  return Object.fromEntries(
    keys.map((key) => {
      ...
      return [
        key,
        typeof value === 'function' ? reactify((value as any).bind(obj), options) : value,
      ];
    }),
  ) as any;
}

...
type UseTimeAgoReturn<Controls extends boolean = false> = Controls extends true
  ? { timeAgoIntl: Signal<string>; parts: Signal<Intl.RelativeTimeFormatPart[]> } & Pausable
  : Signal<string>;
...
export function useTimeAgoIntl(
  time: Signal<Date> | Date | Signal<number> | number | Signal<string> | string,
...
export function useTimeAgoIntl(
  ...
  const parts = computed(() => result().parts);
  ...
  return exposeControls ? ({ timeAgoIntl, parts, ...controls } as any) : timeAgoIntl;
}

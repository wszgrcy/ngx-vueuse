...
function getDefaultScheduler(options: UseMemoryOptions) {
  if ('interval' in options || 'immediate' in options || 'immediateCallback' in options) {
    const { interval = 1000, immediate, immediateCallback } = options;
...
export interface UseMemoryReturn extends Supportable {
  memory: Signal<MemoryInfo | undefined>;
}
...
export function useMemory(options: UseMemoryOptions = {}): UseMemoryReturn {
  const memory = signal<MemoryInfo | undefined>(undefined);
  const isSupported = useSupported(
    () => typeof performance !== 'undefined' && 'memory' in performance,
  );

  if (isSupported()) {
    const { scheduler = getDefaultScheduler } = options;

    scheduler(() => {
      memory.set((performance as PerformanceMemory).memory);
    });
  }

...
import { toValue, tryOnMounted, tryOnUnmounted } from '@cyia/ngx-vueuse/shared';
...
export interface UseScriptTagReturn {
  scriptTag: Signal<HTMLScriptElement | null>;
...
export function useScriptTag(
  src: Signal<string> | string | (() => string),
  ...
  const unload = () => {
    ...
    if (scriptTag()) scriptTag.set(null);
